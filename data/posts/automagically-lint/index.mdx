...
<Aside>
  If you prefer to set these rules via a `.prettierrc` file, that's possible too.
</Aside>

<Aside>
 Prettier rules specified in the `.eslintrc` file like this will overwrite ones in a `.prettierrc` file.
</Aside>
...
I like semicolons, so I changed the `"prettier/prettier"` rule to require them now.
