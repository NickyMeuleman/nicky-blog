...
import { GOLDemo, JSRuntimeChart, JSWASMTime, Timeshift } from "./";
...
The [Speed, Speed, Speed talk by Franziska Hinkelmann](https://www.youtube.com/watch?v=uMuYaES4W3o&t=739s) goes into this.
...
The answer is the very unsatisfying "it depends", but I've heard numbers of about 1.2x native speed.

The end result is that WASM takes less time to get going than JavaScript, and once it gets going, it'll probably be faster.

Below is a bar chart detailing roughly the total time each step takes during the total runtime of a file.
...
Those solutions might not exist at all in JavaScript while there are packages in those other languages.
...
And in the hypotethical scenario where they did rewrite everything in JavaScript, it would likely be too slow, bringing us back to the previous point, WASM can go [fast](#fast).
...
It does this by targeting a virtual machine, and letting the physical machines handle the translation of those universal instructions to machine code.

This is also often called [portability](https://webassembly.org/docs/portability/).
...
I'd like to change [that popular JavaScript quote](https://en.wikipedia.org/wiki/Jeff_Atwood#:~:text=In%202007%2C%20Jeff%20Atwood%20made,question%2Dand%2Danswer%20website.) and make it about WASM.
...
WebAssembly is sandboxed, just like JavaScript is. It can’t access the user’s operating system directly
...
WASM is very bare-bones, that means that for fairly simply functionality, a JavaScript file can be smaller/faster than WASM,
...
I'm actively learning Rust at the moment, and I know very little about other languages, so most examples are focussed on Rust.
...
The following is that thing, the Rust is fairly similar, but the JavaScript is not.
You can judge my scrappy JavaScripting chops on [GitHub because my blog is opensource](https://github.com/NickyMeuleman/nicky-blog).
...
It's a bit different from the one in the book, it should run a bit faster because of some optimizations I made.
If you want to hear about those, [ping me on Twitter](https://twitter.com/NMeuleman).
You can also look at the code, because I publicly [share what I'm up to while learning Rust](https://github.com/NickyMeuleman/scrapyard/tree/main/rust_wasm_book).

The driver logic is implemented in the same thing my blog uses, [React](https://reactjs.org/).

The calculation logic where the new state of the board is calculated, that's done in [Rust](https://www.rust-lang.org/), compiled to WASM with [wasm-pack](https://rustwasm.github.io/wasm-pack/).

The React part uses the glue code wasm-pack provides to call into the WASM.

TODO: explain glue code in the HOW section and use a diagram showing there's glue on both sides
...
To me, the most important thing about Jamstack is time shifting.
...
Compiling ahead of time instead of doing that clientside like JavaScript does.

Doing more work ahead of time, so there has to be less work done when the code you wrote is being used.

Serverless is also a big usecase for WASM.
Running `.wasm` files directly means many languages can be used.
...
The JavaScript runtime has to start up first, and that takes time.
...
but as the vast majority of serverless functions have a very low runtime, so the startup time is significant.
...
I'm a greedy person, I always want more.
...
I can write code, and have it be executed much closed to users by using an edge network.
Those are tiny servers all over the world that typically serve static files like HTML, CSS, and JS to users.
...
notes: investigate https://suborbital.dev/.
