...
Because the compiled WASM comes directly from a host language like Rust, the WASM runs at **near-native** speed of that host language.
...
The post where [Surma](https://twitter.com/dassurma) details [replacing a hot path in your app's JavaScript with WASM](https://developers.google.com/web/updates/2019/02/hotpath-with-wasm) is really good.
...
Because a WASM file is often compiled,
...
This probably means trading a little more compile time for smaller sizes though, a [tradeoff](https://docs.rust-embedded.org/book/unsorted/speed-vs-size.html) that is left to the user to decide.
...
The web isn't the only place WASM run though, it has many exciting applications on traditional servers, in the cloud, in IoT devices.

I'd like to change that popular JavaScript quote and make it about WASM.
Anywhere with a processor WASM _can_ run on, WASM _will_ run on.
...
If you compile to WASM instead, it will run slightly slower, but it will just crash when the pointer is wrong instead of causeing that error that can lead to stealing your stuff.
...
To me, the most important thing to Jamstack is time shifting.
