...
### Deleting non-existent tracing branches

You might have branches locally that have since been deleted remotely.

`git remote prune <remote>`

This command will delete local branches that were set up to follow remote branches if that remote branch has been deleted.
In my example project that only has one remote named "origin" that becomes
