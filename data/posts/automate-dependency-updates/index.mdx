...
This translates to a `.json` file with a single line in it.
...
This is the point in time where spending some time on the [renovate documentation site](https://docs.renovatebot.com/) is helpful.
...
If you use a [lockfile](https://classic.yarnpkg.com/en/docs/yarn-lock/) like `yarn.lock` or `package-lock.json`, then merging one means changing your lockfile.
...
This automates a part of the process of staying up to date with packages, but still creates a _lot_ of noise.

Merging all those PRs can get overwhelming, and could also start to feel like a chore. Putting us back at square 1, that's what we wanted to avoid in the first place!
...
It is possible to go even further by more agressively grouping things.  
...
If the tests fail, a PR is made.
...
First, only certain types of package updates are automatically merged. Not the absence of `"major"` in that first `"updateTypes"` array.
