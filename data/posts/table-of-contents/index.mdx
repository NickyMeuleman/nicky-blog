...
Each `item` we previously rendered, can have another `items` array within it.
...
```jsx hl=9 title=TableOfContents.js
function renderItems(items) {
  return (
    <ol>
      {items.map((item) => (
...
An `IntersectionObserver` works by first setting up the logic for it first, and then telling it to start watching an element (eg. an anchor tag).
...
Getting references to all heading element links in the table of contents can be done by calling [`document.getElementById()`](https://developer.mozilla.org/nl/docs/Web/API/Document/getElementById) with each heading's CSS-id.
...
This hook returns a piece of state that holds the id of the heading that's currently active and returns that piece of state.
...
Back in the `<TableOfContents />` component, the active id can be used to change the styling of the active heading.
...
Inside the `renderItems` function, check if the id of the current element is the same as the one passed in as `activeId` and adjust the styling accordingly.
