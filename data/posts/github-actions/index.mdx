...
<!-- image of the workflow on GitHub -->

## A bit more advanced (and serious)

This one automatically builds and deploys my website on every push to the master branch.
...
To build (and afterwards deploy) the website, some tools and setup is needed.
...
By default, every **job** runs in parallel, but they can be configured to depend on another job and run in serial.

[Benjamin Lannon](https://twitter.com/lannonbr) made a lightbulb change color to green or red, based on the success of a step in a workflow.

### Create your own Action

While there are a bunch of Actions already out there, the best ones are always the ones you made yourself.
