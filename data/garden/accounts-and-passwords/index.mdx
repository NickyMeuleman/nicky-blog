...
Or, a way to say it that's at least twice as entertaining: Have I been pwned?

The website [haveibeenpwned.com](https://haveibeenpwned.com/) tries to answer that question.

Whenever there's a big breach or leak of account details, this website keeps track of it.
...
It will do it without ever receiving that password. Whaaaaat 🤯.

<Aside variant="info">

You don't have to type in your password on their website.
They have a [REST-API](https://haveibeenpwned.com/API/v3) which can get that same information.
...
- You first hash your password and send them a part of that hash.
- They then send back a bunch of potential matches (the hashes that start with the same characters you gave them), and the amount of times each hash was found in a database full of cracked passwords.
