...
If `my_pick` held yet another number or if it held `None`, the `else` block would execute.
...
The thing we try to destructure doesn't have to be the kind of enum variant that holds something, simple enums work too!
