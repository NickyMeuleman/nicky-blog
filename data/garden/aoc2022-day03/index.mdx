...
- Each elf if carrying an even number of items.
...
As with day2, I'll use the fact that for both `a` to `z` and `A` to `Z` every value in that sequence has an incrementing [ASCII](https://en.wikipedia.org/wiki/ASCII) values.
