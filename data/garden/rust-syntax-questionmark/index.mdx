...
- If the returned value is an `Ok`, `s` is wrapped in an `Ok` and returned from the `match`.
...
The `?` operator can be used when operating on a `Result` type if the function it's in returns a `Result` as well.

Every time you see a `?`, that's a possible early return from the function the `?` is used in.

The `?` is a shorthand for those match statements, it operates on the `Result` values.
...
If there is no error, execution of the function continues.
...
It will send the `error` contained in `Err(error)` through the [`From::from` function](https://doc.rust-lang.org/std/convert/trait.From.html#tymethod.from).
ie. if the `Result` variant the `?` is called on is `Err(error)`, the function will return `Err(From::from(error))`.
...
`?` can also be used on the `Option` type, it will then propagate the `None` variant of the `Option`.

```rust
fn get_number(name: String) -> Option<String> {
 if name == "Taylor" {
     Some(String::from("1-800-TAYLOR"))
 } else {
     None
 }
}

fn call_me_maybe(name: String) -> Option<String> {
    let number = get_number(name)?;
    // calling the number
    Some(String::from("successfully called"))
}
```
