...
Every instruction completes before an other one is executed.
...
The `X` register controls horizontal middle of a 3 block wide sprite.

There is no concept of vertical position.

- The screen and the CPU are tied to the same clock cycles.
- The screen draws a pixel every cycle. It's position incrementing by one every cycle.
...
Some constant that are mentioned in the question:
...
It draws left to right, top to button.
Every time it begins drawing a new row, it starts at the left edge again.

<Aside variant="danger">
    The `cycle` starts at 1, and the index on the `screen` start at 0!
</Aside>
...
We represent the screen as a big a array of length `ROWS * COLS`.
...
My screen is larger so I split the array up in chunks of `COLS`, converted each chunk to a string, and joined those strings with a newline.
