...
import { aoc2023Machine } from "../machines/aoc2023";
...
function getCodeLink(day) {
  ...
  return `https://github.com/NickyMeuleman/scrapyard/blob/main/advent_of_code/2023/solutions/src/day_${padded}.rs`;
}

const AoC2023Solver = ({ data }) => {
  console.log(`aoc2023 page loaded`);
  const [state, send] = useMachine(aoc2023Machine);
        ...
        <DemoArea title="Input area">
          <Input title="Day">
            ...
            >
              ...
              {/* change days available through UI here */}
              {Array(2)
                .fill()
                .map((_, idx) => {
...
export const aoc2023Query = graphql`
  query aoc2023Query {
    file(name: { regex: "/aoc2023/" }) {
      childImageSharp {
        original {
          src
        }
