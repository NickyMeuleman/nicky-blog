...
const PostsPage = ({ data, pageContext }) => {
  const { isFirst, isLast, pageIndex, numPages } = pageContext;
  const previousPageNum = pageIndex - 1 === 1 ? '' : (pageIndex - 1).toString();
  const nextPageNum = (pageIndex + 1).toString();
  return (
    <Layout>
      <Container>
        <Triangle>
        ...
        <Pagination>
          {!isFirst && (
            <div>
              <PaginationLink first={pageContext.isFirst} url={`/blog/${previousPageNum}`} text="Go to Previous Page" />
            </div>
          )}
          ...
          {!isLast && (
            <div>
              <PaginationLink last={pageContext.isLast} url={`/blog/${nextPageNum}`} text="Go to Next Page" />
            </div>
          )}
        </Pagination>
        page context:
