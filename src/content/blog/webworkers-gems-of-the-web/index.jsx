...
import partyCorgi from "./party-corgi.gif";
const partyCorgiSrc = partyCorgi.src;
...
const DemoArea = ({ title, children }) => {
  return (
    <div className="mb-2 flex-1 border border-watermarkBg p-4">
      <p className="m-0 mb-1 text-sm font-bold uppercase tracking-wider text-mutedTextBg">
        {title}
      </p>
      <div className="grid grid-flow-dense grid-cols-[repeat(auto-fill,minmax(200px,1fr))] gap-4">
        {children}
      </div>
    </div>
  );
...
const Output = ({ title, children, ...props }) => {
  return (
    <div className="grid grid-rows-[min-content] text-mutedText" {...props}>
    ...
    <React.Fragment>
      ...
      <DemoArea>
        <Output title="JS Animation">
          <div
            ref={jsAnimEl}
            className="min-h-16 min-w-4 rotate-[--degrees] self-center justify-self-center rounded-sm bg-primary"
          />
        </Output>
        <Output title="GIF">
