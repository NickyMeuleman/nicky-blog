...
- Constants can only be set to a constant [expression](/garden/rust-expression-statement), not to the result of a function call or anything that could only be determined at runtime.

As a result, constants are always fixed in size, and known at compile time.  
Another, less obvious result, is that constants may not be of a type that requires [allocation on the heap](/garden/rust-stack-heap), since they're not known at compile time.
